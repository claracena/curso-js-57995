...
*  Entrega final                                
*                                               
*  El siguiente es el codigo principal del      
*  trabajo realizado por Cesar Aracena          
*  durante el curso y pretende ser una pagina   
*  de ayuda para armar correctamente una PC,    
*  filtrando y mostrando solo los componentes   
*  de cada categoria que sean compatibles con lo
*  ya seleccionado                              
*
**************************************************/

// Funcion que derivara al usuario a Google en caso de
// no aceptar los "terminos" iniciales
function uir() {
    window.open('https://www.google.com', '_self');
}

// Funcion que muestra el modal en la primer visita de cada sesion
// y en las visitas subsecuentes salvo que deje seleccionada
// la opcion de no volver a mostrarlo *durante esa sesion)
...
// Funcion para cerrar el modal, guardando la seleccion en sessionStorage
...
// Se realiza un fetch por cada endpoint y se almacenan como objetos en una constante
const llamadas = endpoints.map((ruta) => fetch(ruta).then((respuesta) => respuesta.json()));

// Se realizan las promesas para traer los datos de todos los endpoints
// y se asignan a sus variables correspondientes como datos crudos
...
// Se hace un loop para estar seguro de que "lleguen" todos los datos
const inicializacion = setInterval(() => {
    ...
    // Se comienza a llenar el DOM para la primer opcion
    ...
    // Inmediatamente se comienza a escuchar los eventos del DOM
    ...
    // Lleno las opciones de la placa de video desde ahora
    // ya que no depende de ninguna otra seleccion
    filtered_gpu = gpu_raw_data;

    // Si se cargaron todos los objetos, finalizao el loop y
    // permito comenzar con la seleccion
    if (
...
// Funcion para resetear todo o bien solo uno de los componentes
...
// Funcion para habilitar el siguiente paso sin importar cual sea
...
// Funcion para calcular los watts que utilizara la PC
function wattage() {
    min_wattage = 0;
    max_wattage = 0;
...
// Funcion para mostrar los detalles a la derecha (o abajo en moviles)
function show_info_box() {
    ...
    if (selected_psu.length > 0) {
        ...
        // Disparador de un Sweet Alert una vez que se finaliza
        Swal.fire({
            title: 'Build Completa!',
            text: 'Felicitaciones! En el cuadro de "Detalle" tiene la lista de los componentes para armar su PC.',
            icon: 'success',
...
// Evento de seleccion de motherboard
...
// Evento de seleccion de memoria
...
// Evento de seleccion de cantidad de memoria
selection_ram_qty.onchange = function (e) {
    selected_ram_qty = this.value;
    show_info_box();
};

// Evento de seleccion de disco m.2
...
// Evento de seleccion de cantidad de disco m.2
selection_m2_qty.onchange = function (e) {
    selected_m2_qty = this.value;
    show_info_box();
};

// Evento de seleccion de disco solido
...
// Evento de seleccion de cantidad de disco solido
selection_ssd_qty.onchange = function (e) {
    selected_ssd_qty = this.value;
    show_info_box();
};

// Evento de seleccion de placa de video
...
// Evento de seleccion de fuente de poder
